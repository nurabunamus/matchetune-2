...
export class VideosComponent implements OnInit {
  ...
  async getData() {
    ...
    try {
      let getDataVideos = await getDocs(q);
      this.lastIndex = getDataVideos.docs[getDataVideos.docs.length - 1];
      console.log(getDataVideos.docs.length);
      if (getDataVideos.empty) {
        this.empty = true;
        this.loader = false;
      }
      getDataVideos.forEach((video: any) => {
