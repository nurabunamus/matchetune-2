...
export class InfographicsComponent implements OnInit {
  ...
  async getData() {
    ...
    try {
      let getDataInfogs = await getDocs(q);
      this.lastIndex = getDataInfogs.docs[getDataInfogs.docs.length - 1];
      console.log(getDataInfogs.docs.length);
      if (getDataInfogs.empty) {
        this.empty = true;
        this.loader = false;
      }
      getDataInfogs.forEach((image: any) => {
        let doc = { id: image.id, ...image.data() };
